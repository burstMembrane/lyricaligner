...
def detect_timing_drift(words: List[Word], max_drift_rate: float = 0.05) -> bool:
    ...
    if len(gaps) > 5:
        ...
        if late_gaps > early_gaps * (1 + max_drift_rate):
            logger.warning(
                f"Timing drift detected: early gaps {early_gaps:.3f}s, late gaps {late_gaps:.3f}s"
            )
            return True
...
def calculate_dynamic_frame_duration(
    audio_length: float, total_frames: int, model_frame_duration: float
) -> float:
    """
    ...
    if abs(correction_factor - 1.0) > 0.01:  # Log if correction > 1%
        logger.info(
            f"Frame duration corrected: {model_frame_duration:.6f}s -> {corrected_duration:.6f}s "
...
def smooth_timing_with_confidence(
    words: List[Word], confidence_threshold: float = 0.5
) -> List[Word]:
    """
    ...
    for i in range(len(anchors) - 1):
        ...
        if end_idx - start_idx > 1:  # Only interpolate if there are words in between
            ...
            for j, word in enumerate(
                segment_words[:-1]
            ):  # Skip last word (it's the anchor)
...
def apply_timing_anchors(
    words: List[Word], anchor_interval: float = 10.0
) -> List[Word]:
    """
    ...
    for i, anchor_time in enumerate(anchor_times[1:-1], 1):  # Skip first and last
        ...
        closest_idx = min(
            range(len(words)), key=lambda x: abs(words[x].start - anchor_time)
        )
